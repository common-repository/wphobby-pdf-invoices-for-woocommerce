    ...
    class WHPDF_Admin {
        ...
        */
        ...
        public function whpdf_register_menu(){
            $upgrade_menu_text = '<span class="robot-upgrade-submenu">'.__( 'Upgrade &nbsp;&nbsp;➤', 'wphobby-woo-pdf-invoice' ) . '</span>';

            add_menu_page( 'WooCommerce PDF Invoice Maker', 'PDF Invoices', 'manage_options', 'whpdf-welcome', array( $this, 'whpdf_panel_welcome' ), WHPDF_URL . '/assets/images/icon.svg', '2');
            add_submenu_page('whpdf-welcome', 'Settings', 'Settings', 'manage_options', 'whpdf-panel', array( $this, 'whpdf_panel_general' ) );
            add_submenu_page('whpdf-welcome', 'Help & Guide', 'Help & Guide', 'manage_options', 'whpdf-help', array( $this, 'whpdf_panel_help' ) );
            add_submenu_page('whpdf-welcome', 'Upgrade', $upgrade_menu_text, 'manage_options', 'whpdf-upgrade', array( $this, 'whpdf_panel_upgrade' ) );
         ...
         * @since 1.0.0
         */
        public function whpdf_panel_welcome() {
            ?>
            <div class="robot-getting-started">
                <div class="robot-getting-started__box postbox">
                    <div class="robot-getting-started__content">
                        <div class="robot-getting-started__content--narrow">
                            <h2><?php esc_html_e( 'Welcome to WooCommerce PDF Invoices Maker', 'wphobby-woo-pdf-invoice' ); ?></h2>
                            <p><?php esc_html_e( 'Get introduced to WooCommerce PDF Invoices Maker by watching our "Getting Started" video series. It will guide you to create your website.', 'wphobby-woo-pdf-invoice' ); ?></p>
                        </div>

                        <div class="robot-getting-started__video">
                            <iframe width="620" height="350" src="<?php echo esc_url( 'https://www.youtube.com/embed/AKwP7HmI_sY'); ?>" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen=""></iframe>
                        </div>

                        <div class="robot-getting-started__actions robot-getting-started__content--narrow">
                            <a href="<?php echo esc_url('https://1.envato.market/62LDr') ?>"
                               target="_blank"
                               class="button button-primary button-hero">
                                <?php esc_html_e( 'Premium Version', 'wphobby-woo-pdf-invoice' ); ?>
                            </a>
                            <a href="<?php echo esc_url('http://pdf.wphobby.com/document/general-settings/') ?>"
                               target="_blank"
                               class="button button-secondary button-hero">
                                <?php esc_html_e( 'Document', 'wphobby-woo-pdf-invoice' ); ?>
                            </a>
                        </div>
                    </div>
                </div>
         ...
         * @since 1.0.0
         */
        public function whpdf_panel_upgrade() {
            ?>
            <div class="whpdf-panel">
                <div class="wrap">
                    <?php require_once( WHPDF_DIR . '/includes/admin/sections/upgrade/top.php' ); ?>
                    <?php $this->whpdf_display_message(); ?>
                    <?php
                        require_once( WHPDF_DIR . '/includes/admin/sections/upgrade/content.php' );
                    ?>
                </div>
            </div>
            <?php
        ...
        public function whpdf_shop_address_output() {
            $options = get_option( 'whpdf_general_data' );
            ?>
            <textarea name="whpdf_general_data[whpdf_field_shop_address]"><?php echo isset($options['whpdf_field_shop_address']) ? esc_html($options['whpdf_field_shop_address']) : '';?></textarea>
            <?php
        }
        ...
        public function whpdf_shop_logo_output() {
            ...
            if ( !empty( $options['whpdf_field_shop_logo'] ) ) {
                $image_attributes = wp_get_attachment_image_src( $options['whpdf_field_shop_logo'] );
        ...
        public function whpdf_attach_email_output() {
            ...
            <div class="whpdf_attach_email">
              ...
              <p><input type="checkbox"  value="<?php echo esc_attr($value); ?>" name="whpdf_advanced_data[whpdf_field_attach_email]['complete']" <?php echo isset($options['whpdf_field_attach_email']['complete']) && $options['whpdf_field_attach_email']['complete']== '1' ? 'checked' : '';?>>Completed Order</p>
